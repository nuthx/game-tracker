...
export async function closeErrorRecord() {
  ...
  console.log(`关闭了${records.length}条错误记录`);
}
...
export async function checkPresence() {
  try {
    ...
    // 获取用户在线状态
    const presence = await getBasicPresence(authorization, user.psnId);
    ...
    await prisma.psnRecord.create({
      data: {
        ...
        format: game.format,
        launchPlatform: game.launchPlatform,
        conceptIconUrl: game.conceptIconUrl,
        startAt: new Date(),
        user: {
          connect: { id: user.id }
        }
    ...
    console.log(`正在游玩：[${game.launchPlatform} - ${game.npTitleId}] ${game.titleName}`);
  } catch (error) {
    console.error(error);
  }
}
